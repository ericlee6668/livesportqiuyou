...
public class MainGameBasketballListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private Gson gson = new Gson();
    private Context mContext;
    private List<MatchBean> gameBasketBallMatchBeanList;
    private RequestOptions options = new RequestOptions()
            .placeholder(R.mipmap.icon_default_logo)
            .error(R.mipmap.icon_default_logo);


    public MainGameBasketballListAdapter(Context context) {
        this.mContext = context;
    }

    @Override
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.layout_game_basketball_list_item, parent, false);
    ...
    public void setListData(List<MatchBean> gameBasketBallMatchBeanList) {
        this.gameBasketBallMatchBeanList = gameBasketBallMatchBeanList;
        notifyDataSetChanged();
    }

    public void addListData(List<MatchBean> gameFootBallMatchBeanList) {
        if (this.gameBasketBallMatchBeanList == null) {
            this.gameBasketBallMatchBeanList = gameFootBallMatchBeanList;
        } else {
            this.gameBasketBallMatchBeanList.addAll(gameFootBallMatchBeanList);
        }
        notifyDataSetChanged();
    }
    ...
    private class RecyclerViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void bind(int position) {
            ...
            try {
                if (gameBasketBallMatchBeanList != null && !gameBasketBallMatchBeanList.isEmpty()) {
                    MatchBean gameBasketBallMatchBean = gameBasketBallMatchBeanList.get(position);
                    tv_time.setText(TimeUtil.timeToHour(gameBasketBallMatchBean.getMatchTime() * 1000));
                    tv_game_name.setText(gameBasketBallMatchBean.getLeague() == null ? "" : gameBasketBallMatchBean.getLeague().getNameZh());
//                    tv_game_wheel.setText(gameBasketBallMatchBean.get);
                    tv_a_score.setText(getScore(gameBasketBallMatchBean.getHomeScores()));
                    tv_b_score.setText(getScore(gameBasketBallMatchBean.getAwayScores()));
//                    1未开赛，2比赛中，3已完赛
                    if (gameBasketBallMatchBean.getIsPlaying() == 2) {
                        long matchConductTime = gameBasketBallMatchBean.getUpdated_time() - gameBasketBallMatchBean.getMatchTime();
                        tv_game_des.setText(gameBasketBallMatchBean.getState_str() + matchConductTime / 60 + ":" + matchConductTime % 60);
                        ...
                        iv_ball.setImageResource(R.mipmap.icon_basketball_ing);
                    ...
                    } else if (gameBasketBallMatchBean.getIsPlaying() == 3) {
                        tv_game_des.setText("-");
                        tv_game_des.setTextColor(mContext.getResources().getColor(R.color.black2));
                        iv_ball.setImageResource(R.mipmap.icon_basketball_gray);
                        ...
                        tv_play_status.setText(TextUtils.isEmpty(gameBasketBallMatchBean.getState_str()) ? mContext.getString(R.string.game_not_start) : gameBasketBallMatchBean.getState_str());
                        tv_game_name.setTextColor(mContext.getResources().getColor(R.color.black2));
                        iv_play_status.setImageResource(R.mipmap.icon_ball_not_start);
                    }

                    Team home_team = gameBasketBallMatchBean.getHome_team();
                    Team away_team = gameBasketBallMatchBean.getAway_team();
        ...
        public void onClick(View v) {
            if (gameBasketBallMatchBeanList != null && !gameBasketBallMatchBeanList.isEmpty()) {
                ...
                intent.putExtra(Constants.LIVE_MATCH_BASKETBALL_INFO, gameBasketBallMatchBeanList.get(getAdapterPosition()));
    ...
    public void setUpdateData(List<Object> gameBasketballMatchBean) {
        try {
            if (gameBasketballMatchBean != null && gameBasketBallMatchBeanList != null && !gameBasketBallMatchBeanList.isEmpty()) {
                AppLog.e("赛事列表收到通知：刷新篮球列表");
                for (int i = 0; i < gameBasketBallMatchBeanList.size(); i++) {
                    for (int k = 0; k < gameBasketballMatchBean.size(); k++) {
                        MatchBean gameBasketballMatch = (MatchBean) gameBasketballMatchBean.get(k);
                        if (gameBasketballMatch.getId() == gameBasketBallMatchBeanList.get(i).getId()) {
                            AppLog.e("赛事列表收到通知：刷新篮球列表，匹配上ID：" + gameBasketballMatch.getId());
                            gameBasketBallMatchBeanList.get(i).setIsPlaying(gameBasketballMatch.getIsPlaying());
                            gameBasketBallMatchBeanList.get(i).setHomeScores(gameBasketballMatch.getHomeScores());
                            gameBasketBallMatchBeanList.get(i).setAwayScores(gameBasketballMatch.getAwayScores());
                        }
                    }
                }
                notifyDataSetChanged();
