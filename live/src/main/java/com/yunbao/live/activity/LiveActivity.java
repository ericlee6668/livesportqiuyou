...
public abstract class LiveActivity extends AbsActivity implements SocketMessageListener, LiveShareDialogFragment.ActionListener, KeyBoardHeightChangeListener, AbsDialogFragment.LifeCycleListener {
    ...
    public void onNewChat(LiveNewChatBean bean) {
        if (mLiveChatViewHolder != null) {
            mLiveChatViewHolder.insertChat(bean);
        }

    }
    @Subscribe(threadMode = ThreadMode.MAIN)
    public void onDeleteChatEvent(DeleteChatEvent e) {
        if (mLiveChatViewHolder != null) {
            mLiveChatViewHolder.deleteChat(e.getId());
        }
    }
...
//        }
    ...
    public void openChatWindow(String atName) {
        ...
        if (mLiveChatViewHolder != null) {
            mLiveChatViewHolder.chatScrollToBottom();
        }
...
//        fragment.show(getSupportFragmentManager(), "LiveChatRoomDialogFragment");
     ...
     * 新版本聊天
     */
    public void sendNewChatMessage(String content) {

        if (content.length() > 30) {
            ToastUtil.show("发言不能超过30个字");
            return;
        }

        WebSocketChatUtil.sendMessage(mWebSocketClient, CommonAppConfig.getInstance().getUid(), mLiveUid, content, Constants.CHAT_SEND_MSG);
...
//    }
    ...
    public void setConfigDisplay(View playContainer, boolean isBack) {
        ...
        if (ori == Configuration.ORIENTATION_LANDSCAPE) {
            ...
            playContainer.getLayoutParams().width = ViewGroup.LayoutParams.MATCH_PARENT;
            playContainer.getLayoutParams().height = UIUtil.dip2px(this, 264);
            ...
            if (isBack) {
                ...
                playContainer.getLayoutParams().width = ViewGroup.LayoutParams.MATCH_PARENT;
                playContainer.getLayoutParams().height = ViewGroup.LayoutParams.MATCH_PARENT;
            }
