package com.sportsLive.qy.activity;
...
//                        forwardMainActivity();
    ...
    private void getConfig() {
        ...
        NetLegacy.getTest(new CommonCallback<ConfigBean>() {
            @Override
                ...
                CommonHttpUtil.getConfig(new CommonCallback<ConfigBean>() {
                    ...
                    public void callback(ConfigBean bean) {
                        if (bean != null) {
                            ...
                            if (!TextUtils.isEmpty(bean.domain_api_live))
                                CommonAppConfig.VIDEO_HOST = bean.domain_api_live;
                            if (!TextUtils.isEmpty(bean.domain_api_sports))
                                CommonAppConfig.SPORTS_HOST = bean.domain_api_sports;
                            if (!TextUtils.isEmpty(bean.domain_websocket_chat))
                                CommonAppConfig.WEBSOCKET_CHAT = bean.domain_websocket_chat;
                            if (!TextUtils.isEmpty(bean.domain_websocket_match))
                                CommonAppConfig.WEBSOCKET_MATCH = bean.domain_websocket_match;
                            if (!bean.prevent_blockade_site.isEmpty())
                                SerializeConfig.setDomains(bean.prevent_blockade_site);
                            findViewById(R.id.ll_progress).setVisibility(View.GONE);
                            SpUtil.getInstance().setStringValue("switch",bean.getService_switch());
                            SpUtil.getInstance().setStringValue("cs",bean.getService_url());
                            SpUtil.getInstance().setStringValue("jtype",bean.getService_jump_type());
                            SpUtil.getInstance().setStringValue("btitle",bean.getService_title());
                            ...
                            if(!Utils.isDestroy(LauncherActivity.this)) {
                                DialogUitl.showLogoTipDialog(LauncherActivity.this, getString(com.yunbao.live.R.string.dialog_tip), getString(R.string.load_failure_2), true,
                                        true, true, false, (dialog, content) -> finish());
                            }
                        }
                    }
                });
...
//        MainActivity2.forward(mContext);
        finish();
    }
