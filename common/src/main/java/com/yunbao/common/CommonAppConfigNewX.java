...
public class CommonAppConfigNewX {
    //Http请求头 Header
    public static final Map<String, String> HEADER = new HashMap<>();
    //域名
    public static final  List<String>listURL=new ArrayList<>();

    public static final List<String> HOSTS = getHostsX();
    public static String HOST = getHost();

    public static String VIDEO_HOST = getVideoHost();
    public static String SPORTS_HOST = getSportsHost();
    public static String WEBSOCKET_MATCH = getWebSocketMatch();
    public static String WEBSOCKET_CHAT = getWebSocketChat();
    //外部sd卡
    ...
    private static CommonAppConfigNewX sInstance;

    private CommonAppConfigNewX() {

    }

    public static CommonAppConfigNewX getInstanceX() {
        if (sInstance == null) {
            synchronized (CommonAppConfigNewX.class) {
                if (sInstance == null) {
                    sInstance = new CommonAppConfigNewX();
                }
            }
        }
        return sInstance;
    ...
    public String getPackageName() {
        try {
            ...
            PackageInfo packageInfo = packageManager.getPackageInfo(
                    CommonAppContext.sInstance.getPackageName(), 0);
    ...
    public static boolean isYunBaoApp() {
        ...
        if (!TextUtils.isEmpty(packageName)) {
            return packageName.contains("com.xiaomi.sportsLive");
        }
        return false;
    }
    ...
    public String getAppName() {
        if (TextUtils.isEmpty(mAppName)) {
            int res = CommonAppContext.sInstance.getResources().getIdentifier("app_name", "string", getPackageName());
    ...
    public static List<String> addHostsX(String url) {
    ...
    private static List<String> getHostsX() {
        String host = getMetaDataString("SERVER_HOST");
        Log.i("CommonAppConfig", "host str is = " + host);
    ...
    private static String getVideoHost() {
        String host = getMetaDataString("VIDEO_SERVER_HOST");
        HEADER.put("referer", host);
        return host;
    }

    private static String getSportsHost() {
        String host = getMetaDataString("SPORTS_SERVER_HOST");
        HEADER.put("referer", host);
        return host;
    }

    public static String getWebSocketMatch() {
        String host = getMetaDataString("WEBSOCKET_MATCH");
        return host;
    }

    public static String getWebSocketChat() {
        String host = getMetaDataString("WEBSOCKET_CHAT");
        return host;
    }
    public static String getShareLink() {
        String shareLink = getMetaDataString("SHARE_LINK");
        return shareLink;
    }
    /**
     * 个人中心功能列表
    ...
    public void setHuaWei(HuaWeiCloudBean huawei) {
        SpUtil.getInstance().setStringValue(SpUtil.HUAWEI, new Gson().toJson(huawei));
    }

    public HuaWeiCloudBean getHuaWei() {
        String json = SpUtil.getInstance().getStringValue(SpUtil.HUAWEI);
        Gson gson = new Gson();
        return gson.fromJson(json, HuaWeiCloudBean.class);
    }
