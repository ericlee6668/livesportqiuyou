...
public class VideoScrollViewHolder extends AbsViewHolder implements
    ...
    public VideoScrollViewHolder(Context context, ViewGroup parentView, int position, String videoKey, int page, int initType) {
        super(context, parentView, position, videoKey, page, initType);
        this.mInitType = initType;
        if (mIvBack != null) {
            mIvBack.setVisibility(mInitType == Constants.VIDEO_MAIN_PLAY_IN ? View.GONE : View.VISIBLE);
        }
    }
...
//        preLoadThumb(list);
    ...
    //预加载封面
    private void preLoadThumb(List<VideoBean> mList) {
        for (VideoBean videoBean : mList) {
            Glide.with(mContext).load(videoBean.getThumb()).diskCacheStrategy(DiskCacheStrategy.DATA).listener(new RequestListener<Drawable>() {
                @Override
    ...
    public void stopClick() {
        if (mVideoPlayViewHolder != null) {
            mVideoPlayViewHolder.rootClick();
        }
    }
